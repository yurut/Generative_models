...
# set dir 
os.getcwd()
os.chdir("/home/jovyan/playGround/Medium/201901_AE/")
...
class VAE(tf.keras.Model):
    ...
    def encode(self, x): # x 為input tensor 
        h = tf.nn.relu(self.fc1(x)) # tensor 過relu 
    ...
    def call(self, inputs, training = None, mask = None):
        ...
        x_reconstructed_logits = self.decode_logit(z)
...
with tf.device(device):
    ...
    for epoch in range(num_epochs):
        ...
        new_im.save('images_vae/vae_sampled_epoch_%d.png' % (epoch + 1))
        ...
        new_im.save('images_vae/vae_reconstructed_epoch_%d.png' % (epoch + 1))
        plt.imshow(np.asarray(new_im))
        plt.show()
